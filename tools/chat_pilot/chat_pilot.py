...
def plan_task_with_openai(prompt, group_prompt, group_history, bots, tools):
    ...
    """
    # 检查是否需要所有机器人同时回答
    all_bots_speak = st.session_state.bot_manager.get_all_bots_speak()
    if all_bots_speak:
        LOGGER.info("开启了所有机器人同时回答模式，将让所有启用的机器人回答问题")
        enabled_bots = [bot for bot in bots if bot.get('enable', True)]
        results = []
        for bot in enabled_bots:
            results.append({"type": "call_bot", "id": bot['id'], "prompt": "请根据上下文回答用户的问题"})
        return results
